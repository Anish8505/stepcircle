...
exports.updatePublicLocation = exports.removeFriend = exports.declineFriendInvite = exports.refreshFriendsLeaderboard = exports.acceptFriendInvite = exports.createFriendInvite = exports.finalizeDailyRankSnapshots = exports.registerNotificationToken = exports.syncDailySteps = void 0;
...
function isTodayOrAdjacentUtc(dateKey) {
    if (!/^\d{8}$/.test(dateKey))
        return false;
    const requested = Date.UTC(Number(dateKey.slice(0, 4)), Number(dateKey.slice(4, 6)) - 1, Number(dateKey.slice(6, 8)));
...
function dateKeyForNow() {
    const now = new Date();
    return `${now.getUTCFullYear()}${String(now.getUTCMonth() + 1).padStart(2, '0')}${String(now.getUTCDate()).padStart(2, '0')}`;
}
function dateKeyForOffset(time, offsetMinutes) {
...
async function directFriendIds(userId) {
    const snapshot = await db.collection('userFriends').doc(userId).collection('friends').get();
    return snapshot.docs.map((document) => document.id);
}
async function rankInViewerLeaderboard(viewerId, participantId, dateKey) {
...
async function notifyFriendOvertaken(friendId, displayName) {
    const tokens = await db.collection('userNotificationTokens').doc(friendId).collection('tokens').get();
    const values = tokens.docs.map((document) => document.data().token).filter((token) => typeof token === 'string');
    if (values.length === 0)
        return;
    ...
    const removals = result.responses.flatMap((response, index) => response.success ? [] : [tokens.docs[index].ref]);
    if (removals.length > 0)
        await Promise.all(removals.map((reference) => reference.delete()));
}
async function rebuildViewerLeaderboard(viewerId, dateKey) {
    ...
    const participants = await Promise.all(participantIds.map(async (participantId) => {
        ...
        const stepRecord = (stepSnapshot.data() ?? {});
        return {
            participantId,
            profile: (profileSnapshot.data() ?? {}),
            steps: stepRecord.steps ?? 0,
            updatedAt: stepRecord.updatedAt ?? firestore_1.Timestamp.fromMillis(0),
        };
    }));
    participants.sort((left, right) => right.steps - left.steps ||
...
async function rebuildGlobalLeaderboard(dateKey) {
    ...
    const participants = await Promise.all(stepSnapshots.docs.map(async (document) => {
        const stepRecord = document.data();
    ...
    const batch = db.batch();
    for (const document of existing.docs) {
        if (!participantIds.has(document.id))
            batch.delete(document.ref);
    }
    participants.forEach((participant, index) => {
...
exports.syncDailySteps = (0, https_1.onCall)(async (request) => {
    ...
    if (!isTodayOrAdjacentUtc(dateKey)) {
        throw new https_1.HttpsError('invalid-argument', 'Only the current local day may be synced.');
    }
    if (!Number.isInteger(steps) || steps < 0 || steps > maxDailySteps) {
        throw new https_1.HttpsError('invalid-argument', 'Steps must be a valid daily total.');
    }
    if (timeZoneOffsetMinutes < -720 || timeZoneOffsetMinutes > 840) {
        throw new https_1.HttpsError('invalid-argument', 'A valid local time zone offset is required.');
    }
    ...
    await Promise.all(viewers.slice(1).map(async (friendId) => {
        ...
        if (previous?.user == null || previous.friend == null)
            return;
        const [userRank, friendRank] = await Promise.all([
            rankInViewerLeaderboard(friendId, userId, dateKey),
            rankInViewerLeaderboard(friendId, friendId, dateKey),
...
exports.registerNotificationToken = (0, https_1.onCall)(async (request) => {
    const userId = requireUserId(request.auth);
    const token = readString(request.data?.token, 'token');
    if (token.length < 20 || token.length > 512) {
        throw new https_1.HttpsError('invalid-argument', 'The notification token is invalid.');
    }
    const tokenId = (0, node_crypto_1.createHash)('sha256').update(token).digest('hex');
...
exports.finalizeDailyRankSnapshots = (0, scheduler_1.onSchedule)({ schedule: 'every 60 minutes', timeZone: 'Etc/UTC' }, async () => {
    const now = Date.now();
...
exports.createFriendInvite = (0, https_1.onCall)(async (request) => {
    const invitedByUserId = requireUserId(request.auth);
    const invitationCode = (0, node_crypto_1.randomBytes)(18).toString('base64url');
    const expiresAt = firestore_1.Timestamp.fromMillis(Date.now() + 7 * 24 * 60 * 60 * 1000);
...
exports.acceptFriendInvite = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!invite.exists || invite.data()?.status !== 'pending') {
            throw new https_1.HttpsError('not-found', 'This invitation is no longer available.');
        }
        invitedByUserId = invite.data()?.invitedByUserId;
        if (invitedByUserId === acceptingUserId) {
            throw new https_1.HttpsError('invalid-argument', 'You cannot accept your own invitation.');
        }
        ...
        transaction.update(inviteReference, { status: 'accepted', acceptedAt: firestore_1.FieldValue.serverTimestamp() });
    });
...
exports.refreshFriendsLeaderboard = (0, https_1.onCall)(async (request) => {
    const userId = requireUserId(request.auth);
...
exports.declineFriendInvite = (0, https_1.onCall)(async (request) => {
    const userId = requireUserId(request.auth);
    const invitationCode = readString(request.data?.invitationCode, 'invitationCode');
    ...
    await reference.update({ status: 'declined', declinedByUserId: userId, updatedAt: firestore_1.FieldValue.serverTimestamp() });
...
exports.removeFriend = (0, https_1.onCall)(async (request) => {
    ...
    if (!friendship.exists || friendship.data()?.status !== 'accepted') {
        throw new https_1.HttpsError('not-found', 'This direct friendship does not exist.');
    }
    const batch = db.batch();
    batch.update(friendshipReference, { status: 'revoked', updatedAt: firestore_1.FieldValue.serverTimestamp() });
...
exports.updatePublicLocation = (0, https_1.onCall)(async (request) => {
    const userId = requireUserId(request.auth);
